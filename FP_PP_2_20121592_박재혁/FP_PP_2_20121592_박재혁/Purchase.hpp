...
public:
	...
	const char* getKey(const int seed) const
	{
		...
		{
		case seed_both:
			return this->PurchaseID.c_str(); break;
		case seed_lecture:
			return this->LectureID; break;
		case seed_member:
			return this->MemberID.c_str(); break;
		}
	}
	void setKey(const char* key, const int seed)
	{
		...
		{
		case seed_both:
			this->update_purchaseid(key); break;
		case seed_lecture:
			this->update_lectureid(key); break;
		case seed_member:
			this->update_memberid(std::string(key)); break;
		}
	}
